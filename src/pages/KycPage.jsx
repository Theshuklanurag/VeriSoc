...
// Hoisted outside to prevent unmount/remount on every keystroke
...
export default function KycPage() {
    ...
    <div style={{ minHeight: "calc(100vh - 72px)", display: "flex", alignItems: "center", justifyContent: "center", position: "relative", zIndex: 1 }}>
      <div className="card" style={{ textAlign: "center", padding: 48, maxWidth: 480 }}>
        ...
        <div style={{ display: "flex", gap: 12, justifyContent: "center" }}>
          <button className="btn btn-secondary" onClick={() => { navigator.clipboard?.writeText(kycCode); addToast("KYC Code copied!", "success"); }}>
            Copy Code
          </button>
  ...
  const validateStep = () => {
    ...
    if (step === 2) {
      ...
      if (!data.primaryIdNumber.trim()) e.primaryIdNumber = "Enter ID number";
    }
    if (step === 3) {
      if (!data.secondaryIdType) e.secondaryIdType = "Select a different ID type";
      if (!data.secondaryIdNumber.trim()) e.secondaryIdNumber = "Enter ID number";
    }
    if (step === 4) {
  ...
  const submit = async () => {
    ...
    const result = await KycDB.submit({
      ...
      idProofFile:       data.idProofFile,   // actual File object for storage
      selfieFile:        data.selfieFile,    // actual File object for storage
      idProofFileName:   data.idProofFile?.name || "",
      selfieFileName:    data.selfieFile?.name || "",
      digilockerVerified: user.digilockerLinked || false,
      digilockerData:    user.digilockerData || null,
    });
    if (result.error) {
      ...
      <div className="container" style={{ paddingTop: 48, paddingBottom: 80 }}>
        ...
        <div className="card" style={{ maxWidth: 720, margin: "0 auto", padding: "40px 36px" }}>
            ...
            <div>
              ...
              <div className="form-grid">
                ...
                <F name="gender" label="Gender">
                  <select name="gender" value={data.gender} onChange={handle}
            ...
            <div>
              ...
              <div className="form-grid">
                ...
                <F name="state" label="State">
                  <select name="state" value={data.state} onChange={handle}
          ...
          {step === 2 && (
            <div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: 12, marginBottom: 24 }}>
                {ID_TYPES.map(t => (
                  <div key={t.value}
                    onClick={() => { set("primaryIdType", t.value); set("primaryIdNumber", ""); }}
                    style={{
                      padding: "16px 12px", borderRadius: 10, textAlign: "center", cursor: "pointer",
                      border: `1px solid ${data.primaryIdType === t.value ? "var(--gold)" : "var(--border)"}`,
                      background: data.primaryIdType === t.value ? "rgba(201,168,76,0.08)" : "var(--bg2)",
                      transition: "all 0.2s",
                    }}>
                    <div style={{ fontSize: 24, marginBottom: 6 }}>
                      {t.value === "aadhaar" ? "🪪" : t.value === "pan" ? "💳" : t.value === "passport" ? "📘" : t.value === "driving" ? "🚗" : "🗳️"}
                    </div>
                    <div style={{ fontSize: 12, fontWeight: 600, color: data.primaryIdType === t.value ? "var(--gold)" : "var(--text2)" }}>
                      {t.label}
                    </div>
                  </div>
                ))}
              ...
              {data.primaryIdType && (
                <div className="form-group" style={{ marginTop: 8 }}>
                  ...
                  <input
                    name="primaryIdNumber" value={data.primaryIdNumber} onChange={handle}
                    className={`form-input ${errors.primaryIdNumber ? "form-input-error" : ""}`}
                    placeholder={idHint(data.primaryIdType)}
                  />
          ...
          {step === 3 && (
            <div>
              ...
              <p style={{ color: "var(--text2)", fontSize: 14, marginBottom: 24 }}>
                Select a <strong style={{ color: "var(--gold)" }}>different</strong> ID type from your primary document.
              </p>

              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: 12, marginBottom: 24 }}>
                {availableSecondary.map(t => (
                  <div key={t.value}
                    onClick={() => { set("secondaryIdType", t.value); set("secondaryIdNumber", ""); }}
                    style={{
                      padding: "16px 12px", borderRadius: 10, textAlign: "center", cursor: "pointer",
                      border: `1px solid ${data.secondaryIdType === t.value ? "var(--gold)" : "var(--border)"}`,
                      background: data.secondaryIdType === t.value ? "rgba(201,168,76,0.08)" : "var(--bg2)",
                      transition: "all 0.2s",
                    }}>
                    <div style={{ fontSize: 24, marginBottom: 6 }}>
                      {t.value === "aadhaar" ? "🪪" : t.value === "pan" ? "💳" : t.value === "passport" ? "📘" : t.value === "driving" ? "🚗" : "🗳️"}
                    </div>
                    <div style={{ fontSize: 12, fontWeight: 600, color: data.secondaryIdType === t.value ? "var(--gold)" : "var(--text2)" }}>
                      {t.label}
                    </div>
                  </div>
                ))}
              ...
              {data.secondaryIdType && (
                <div className="form-group" style={{ marginTop: 8 }}>
                  ...
                  <input
                    name="secondaryIdNumber" value={data.secondaryIdNumber} onChange={handle}
                    className={`form-input ${errors.secondaryIdNumber ? "form-input-error" : ""}`}
                    placeholder={idHint(data.secondaryIdType)}
                  />
              ...
              <div style={{ marginTop: 28 }}>
                <div style={{ fontSize: 12, fontFamily: "var(--font-mono)", color: "var(--text3)", marginBottom: 16, letterSpacing: 2, textTransform: "uppercase" }}>
                  Optional: Provide additional ID numbers for stronger verification
                </div>
                <div className="form-grid">
            ...
            <div>
              ...
              {[
                ...
                <div key={l} style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", padding: "11px 0", borderBottom: "1px solid var(--border)", fontSize: 14, gap: 16 }}>
                  ...
                  <span style={{ color: "var(--cream)", textAlign: "right" }}>{v || "—"}</span>
                </div>
              ))}
          ...
          <div style={{ display: "flex", gap: 12, marginTop: 32 }}>
            {step > 0 && (
              <button className="btn btn-secondary" onClick={back} style={{ flex: 1 }}>← Back</button>
            )}
            {step < 5 ? (
              ...
              <button className="btn btn-primary" onClick={submit} disabled={submitting} style={{ flex: 2 }}>
                {submitting
                  ? <><span className="spinner" style={{ width: 16, height: 16, marginRight: 8 }} /> Submitting KYC...</>
