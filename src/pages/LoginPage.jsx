...
export default function LoginPage() {
  ...
  // Bug 4 fixed: handle username collision for Google OAuth users
  const findOrCreateGoogleUser = async (gUser) => {
    let user = await UserDB.findByEmail(gUser.email);
    if (user) return user;

    const base = gUser.email.split("@")[0].replace(/[^a-z0-9_]/gi, "_").toLowerCase();
    let username = base;
    let attempt = 1;
    while (true) {
    ...
    const res = await UserDB.create({
      fullname: gUser.user_metadata?.full_name || base,
      email: gUser.email,
      username,
      phone: "",
  ...
  useEffect(() => {
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, session) => {
      if (event === "SIGNED_IN" && session?.user) {
        ...
        try {
          const user = await findOrCreateGoogleUser(gUser);
          if (user) {
            login(user);
          ...
          {showForgot ? (
            <div>
              <div style={{ marginBottom: 20 }}>
                <div style={{ fontWeight: 700, color: "var(--cream)", fontSize: 16, marginBottom: 6 }}>Reset Password</div>
                <div style={{ fontSize: 13, color: "var(--text2)" }}>Enter your email and we'll send a reset link.</div>
              </div>
              {forgotSent ? (
                <div style={{ padding: "14px 16px", background: "rgba(79,192,110,0.08)", border: "1px solid rgba(79,192,110,0.25)", borderRadius: 10, fontSize: 13, color: "var(--success)", marginBottom: 16 }}>
                ...
                <form onSubmit={handleForgotPassword} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                  <div className="form-group">
                    ...
                    <input type="email" value={forgotEmail} onChange={e => setForgotEmail(e.target.value)} className="form-input" placeholder="you@email.com" required />
                  </div>
                  <button type="submit" className="btn btn-primary btn-full" disabled={forgotLoading} style={{ fontSize: 14, padding: "12px", borderRadius: 12 }}>
                    {forgotLoading ? <><span className="spinner" style={{ width: 15, height: 15, marginRight: 8 }} />Sending...</> : "Send Reset Link"}
                  </button>
                </form>
              )}
              <button onClick={() => { setShowForgot(false); setForgotSent(false); setForgotEmail(""); }} style={{ background: "none", border: "none", color: "var(--gold)", cursor: "pointer", fontSize: 13, marginTop: 14, fontFamily: "var(--font-body)" }}>
                ← Back to login
              </button>
            </div>
          ) : (
            <>
              {error && <div style={{ marginBottom: 20, padding: "12px 16px", background: "rgba(201,76,76,0.08)", border: "1px solid rgba(201,76,76,0.25)", borderRadius: 10, fontSize: 13, color: "#e87070" }}>⚠ {error}</div>}
              ...
              <form onSubmit={submit} style={{ display: "flex", flexDirection: "column", gap: 18 }}>
                ...
                <div className="form-group">
                  ...
                  <div style={{ textAlign: "right", marginTop: 6 }}>
                    <span onClick={() => setShowForgot(true)} style={{ fontSize: 12, color: "var(--gold)", cursor: "pointer" }}>Forgot password?</span>
                  </div>
                </div>
                <button type="submit" className="btn btn-primary btn-full" disabled={loading} style={{ marginTop: 2, fontSize: 15, padding: "13px", borderRadius: 12 }}>
